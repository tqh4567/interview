...
public class FourSum18 {
    public static void main(String[] args) {
        FourSum18 fourSum = new FourSum18();
...
//        int[] nums = {-3,-2,-1,0,0,1,2,3};
        System.out.println("fourSum.fourSum(nums,0) = " + fourSum.fourSum(nums, 1057));
    }
    /*
    // 回溯法超时，需要剪枝，怎么剪？
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        if (nums[nums.length-1] + nums[nums.length-2]+nums[nums.length-3]+nums[nums.length-4] < target) return res;
        backTracking(nums,res,list,target,0,visited);
        return res;
    }

    private void backTracking(int[] nums, List<List<Integer>> res, LinkedList<Integer> list, int target, int start, boolean[] visited) {
        ...
        for (int i = start;i < nums.length-1;i++){
            ...
            list.add(nums[i]);
            visited[i] = true;
            backTracking(nums,res,list,target-nums[i],i+1, visited);
            list.removeLast();
            visited[i] = false;
        }
    }
